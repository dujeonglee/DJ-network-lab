...
void arp::print_arp(const unsigned char* const buffer) {
...
int arp::raw_arp_send(const unsigned char* const eth_dst, 
    ...
    unsigned char packet_buffer[ARP_LEN];
...
void arp::start_rcv(const char* const ifname){
    ...
    _worker.PeriodicTask(0, [](){
        ...
        unsigned int remote_addr_length;
        int rx_bytes;

        memset(rx_buffer, 0x00, ARP_LEN);// Initialize the rx buffer.
        memset(&remote_addr, 0, sizeof(remote_addr));
        remote_addr_length = sizeof(remote_addr);
        rx_bytes = recvfrom(_instance->_rcv_socket, rx_buffer, ARP_LEN, 0, (sockaddr*)&remote_addr, &remote_addr_length);// Receive a packet.
        if(rx_bytes == ARP_LEN) {
            printf("Rcv ARP\n");
            _instance->print_arp(rx_buffer);
        }
        return true;
    });
}
